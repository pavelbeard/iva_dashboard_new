      ...
      for (let [innerKey, innerValue] of Object.entries(outerValue)) {
        if (type === "CREATE") {
          ...
          yield (
            <div>
              <b>
                {innerKey} = {innerValue === "" ? "unknown" : innerValue}
              </b>
            </div>
          );
        } else if (type === "UPDATE") {
          ...
          yield (
            <div>
              Параметр <b>{innerKey}</b> изменен с <b>{oldValue}</b> на{" "}
              <b>{newValue}</b>
            </div>
          );
        }
...
export function parseInfo(object) {
  ...
  const conference =
    object["conferenceSessionName"] || object["name"] || object["eventName"];
  const eventType =
    object["changeType"] ||
  ...
  switch (eventType) {
    ...
    case "JOIN":
      return (
        <div>
          Пользователь <b>{username}</b> вошел в конференцию{" "}
          <b>"{conference}"</b> в роли{" "}
          {roles.map((role, n) => {
    ...
    case "CREATE":
      return (
        <div>
          Создано мероприятие <b>{conference}</b> с параметрами:
          <br />
          {Array.from(params(object, eventType)).map((item, n) => (
            <div key={`${item}${n}`}>{item}</div>
          ))}
        </div>
      );
    case "UPDATE":
      return (
        <div>
          Изменены настройки пользователя <b>{username}</b> в мероприятии{" "}
          <b>{conference}</b>
          {Array.from(params(object, eventType)).map((item, n) => {
    ...
    case "LOGOUT":
      return (
        <>
          <div>
            Пользователь <b>{username}</b> вышел из системы <b>{domainName}</b>{" "}
            по причине <b>{sessionEndType}</b>
          </div>
    ...
    case "LEAVE":
      return (
        <div>
          Пользователь <b>{username}</b> вышел из мероприятия{" "}
          <b>{conference}</b> по причине <b>{leaveReason}</b>
        </div>
      );
    case "INCORRECT_EVENT_ID":
      return (
        <div>
          Ошибка при попытке доступа к ресурсу: введен неверный ID мероприятия
        </div>
      );
    case "INVALID_CREDENTIALS":
      return (
        <div>
          Не удалось выполнить вход в домен <b>{domainName}</b>
          <br />с учетными данными{" "}
          <b>{username} из-за неверных учетных данных</b>
        </div>
      );
    case "BLOCKED_ACCESS_FROM_IP":
      return (
        <div>Ошибка при попытке доступа к ресурсу: IP-адрес заблокирован</div>
      );
    ...
    case "NOT_IN_CONFERENCE":
      return (
        <div>
          Попытка неавторизованного выполнения операции в мероприятии{" "}
          <b>{conference}</b>
          <br />
    ...
    case "NO_OPERATOR_ROLE":
      return (
        <div>
          Попытка неавторизованного выполнения операции в администрировании:
          <br />
          отсутствует роль оператора системы
        </div>
      );
    default:
      return "";
