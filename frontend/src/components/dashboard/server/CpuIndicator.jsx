...
const CpuIndicator = ({ host }) => {
  ...
  const setCpuData = async () => {
    try {
      const urlRequest =
        URL +
        `?query=${encodeURI(query.system.cpuData)}` +
        `&host=${host}` +
        `&query_range=false`;
      ...
      if (response.data) {
        ...
        if (0 <= __cpuLoad__ && __cpuLoad__ < 50.0) setColor("#16b616");
        else if (50.0 <= __cpuLoad__ && __cpuLoad__ < 75.0) setColor("#ff9900");
  ...
  const setCpuCores = async () => {
    try {
      const urlRequest =
        URL +
        `?query=${encodeURI(query.system.cpuCores)}` +
        `&host=${host}` +
        `&query_range=false`;
      const response = (await axios.get(urlRequest)).data;
