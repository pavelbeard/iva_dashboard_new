...
const DeviceSsdIndicator = ({ host }) => {
  ...
  const setDeviceSsdData = async () => {
    try {
      const urlRequest =
        URL +
        `?query=${encodeURI(query.disk.rootSpace)}` +
        `&host=${host}` +
        `&query_range=false`;
      ...
      if (response.data) {
        ...
        if (0 <= ssdUsedSpacePrc && ssdUsedSpacePrc < 50.0) setColor("#16b616");
  ...
  return (
    <div className="d-flex flex-row justify-content-start mt-1">
      ...
      <div
        className="ps-2 mt-1"
        data-ivcs-server-attr="filespace"
