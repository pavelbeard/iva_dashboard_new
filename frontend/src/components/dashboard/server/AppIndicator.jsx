...
import { API_URL, parse } from "../../../constants";
...
const AppIndicator = ({ id }) => {
  ...
  // 2 creating list of FALSE which equals entry data
  // btw using localStorage
  ...
  const handleAppMonitoringList = (e, n) => {
    ...
    const appName = parent
      .querySelector('[data-app-name="true"]')
      .textContent.replace(/\|\s+/g, "");
    const status = parent
      .querySelector('[data-app-status="true"]')
      .textContent.replace(/\|\s+/g, "");
  ...
  const autoUpdateAppList = () => {
    ...
    // if app changed its color - changing color
          ...
          {appData.map((app, n = 0) => {
            ...
            return (
              <tr key={key}>
                <td>
                  <input
                    onClick={(e) => handleAppMonitoringList(e, n)}
                    type="checkbox"
                    defaultChecked={checklistStorage[n]}
                    disabled={
                      !checklistStorage[n] &&
                      checklistStorage.filter((bool) => bool === true)
                        .length === 6
                    }
                  />
                </td>
