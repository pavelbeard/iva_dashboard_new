...
const ThreadAppIndicator = ({ host }) => {
  ...
  const getAppsData = async () => {
    try {
      const urlRequest =
        URL +
        `?query=${encodeURI(query.modules.states)}` +
        `&host=${host}` +
        `&query_range=false`;
      ...
      if (response.data) {
        ...
        const threadsCount = sum(
          __appsDataTooltip__.map((i) => {
          ...
          {typeof threadsDataTooltip.map === "function"
            ? threadsDataTooltip.map((i, num = 1) => {
                return (
                  <tr
                    key={
                      i.metric.__name__ + i.metric.groupname + i.metric.state
                    }
                  >
                    <td>{num}</td>
