export const system = {
  cpuData:
    'sum by (mode,instance) (label_keep(rate(node_cpu_seconds_total), "mode")) * 100 / distinct(label_value(node_cpu_seconds_total, "cpu"))',
  cpuCores:
    'label_keep(alias(count(node_cpu_seconds_total{mode="idle"}) without (cpu, mode), "Cpu cores"), "__name__", "device")',
  ...
  memory:
    ...
    '), "__name__")',
  contextSwitches:
    'label_keep(alias(rate(node_context_switches_total), "Context switches"), "__name__")',
  uptime:
    'label_keep((alias((node_time_seconds-node_boot_time_seconds) / 86400, "Uptime")), "__name__")',
};
