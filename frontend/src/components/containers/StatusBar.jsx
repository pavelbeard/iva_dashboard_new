...
import { pingApi, auditLogEvent } from "../../slices/serverSlice";

const StatusBar = () => {
  const dispatch = useDispatch();
  const { apiStatus, ivcsApiStatus, auditLogLastEvents } = useSelector(
        ...
        <div className="text-white">
          <div className="d-flex">
            ...
            <div className="ms-2 status-bar-table-events">
              <div>IP:</div>
              <div style={{ color: color }}>{lastWarningEvent}</div>
              <div>Причина:</div>
              <div style={{ color: color }}>{reason}</div>
            </div>
          </div>
        </div>
      </div>
